...
def load_from_bin(filename:str,n:int,m:int,data_type='i'):
    ...
    with open(filename,'rb') as f:
        binary_data = f.read()
        int_array = struct.unpack(f'{num_elements}{data_type}',binary_data)
...
def saveFrames(n, m, save_path = '/home/domen/Documents/Faks/HPC/Snow-Crystal-Growth/Plotting/frames7/', data_path = '/home/domen/Documents/Faks/HPC/Snow-Crystal-Growth/Plotting/Data'):
    '''to stack the frames use
    ffmpeg -framerate <framerate> -i %d.png <video_name>.<filetype_suffix>
    '''
    a = 1
    ...
    file_names = sorted(file_names, key=numeric_sort)
    clrs = ['#ff8400','#b5b5b5','#00c8ff','#009e25','r','y','b','m']
    mat = load_from_bin(os.path.join(data_path, file_names[0]), n, m)
    plot_hex = MatPlot(mat)
    ...
    for file_name in file_names[1:]:
        mat = load_from_bin(os.path.join(data_path, file_name), n, m)
        print(file_name)
        ttl.set_text(f'Step {cnt+1}')
        plot_hex.update(mat)
        fig.canvas.draw()
        fig.canvas.flush_events()
        cnt += 1
        plt.savefig(f'{save_path}{cnt}.png') # save the figure
...
def strip_the_plane():
    ...
    m = 60
     ...
     # Get the list of files in the "Data" folder
    data_folder = '/home/domen/Documents/Faks/HPC/Snow-Crystal-Growth/Plotting/Data'
    file_names = os.listdir(data_folder)
    file_names = sorted(file_names, key=numeric_sort)
    # Define a regular expression pattern to extract the numeric part from the file names
    pattern = r'array_(\d+)\.bin'

    # Extract the numeric part and sort the file names
    file_names_sorted = sorted(file_names, key=lambda x: int(re.search(pattern, x).group(1)))
    clrs = ['#ff8400','#b5b5b5','#00c8ff','#009e25','r','y','b','m']
    fig,ax = plt.subplots(1,figsize=(10,10))

    # Plot the grids one by one upon keyboard input
    for file_name in file_names_sorted:
        mat = load_from_bin(os.path.join(data_folder, file_name), n, m)
    ...
    """ mat = load_from_bin('/home/domen/Documents/Faks/HPC/Snow-Crystal-Growth/Plotting/Data/array.bin',n,m)#np.zeros((n,m),dtype=int)#
    #mat[1:-1,1:-1] = 1
    fig,ax = plt.subplots(1,figsize=(10,10))
    clrs = ['#ff8400','#b5b5b5','#00c8ff','#009e25','r','y','b','m']
    plot_mat(mat,ax,clr=clrs)
...
if __name__ == "__main__":
    ...
    # create folder and add array.bin files to it.
    # pass data_path (.bin files containing snapshot of each iteration) and save_path (where image files of the simulation will be saved)
    saveFrames(60, 60)
